    ...
    return (
      <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white">
        ...
        <div className="flex-1 flex items-center justify-center">
          <p className="text-xs text-slate-400">Loading verified practitioner profile...</p>
        </div>
        <Footer />
      </div>
    );
    ...
    return (
      <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white">
        ...
        <div className="flex-1 max-w-4xl mx-auto px-4 py-16 text-center space-y-4">
          ...
          <h2 className="text-xl font-bold">Practitioner Record Not Found</h2>
          <p className="text-xs text-slate-400">
            This doctor or clinic ID is not registered in the verified Maharashtra healthcare registry.
          </p>
          <Button onClick={() => router.push("/doctors")} className="bg-teal-600 text-white text-xs">
            Back to Maharashtra Directory
          </Button>
        </div>
        <Footer />
      </div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white transition-colors duration-300">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xs relative overflow-hidden">
          ...
          <div className="flex flex-wrap items-center gap-3 mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
            {doctor.patients_treated && (
              <span className="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-800 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900 px-3 py-1 rounded-xl">
                <Users className="w-3.5 h-3.5 text-emerald-600" />
                {doctor.patients_treated}
              </span>
            )}
            {doctor.years_of_practice && (
              <span className="inline-flex items-center gap-1.5 text-xs font-bold text-indigo-800 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-900 px-3 py-1 rounded-xl">
                <Award className="w-3.5 h-3.5 text-indigo-600" />
                {doctor.years_of_practice}
              </span>
            )}
            {doctor.facility_type_label && (
        ...
        {/* Multiple Practice Establishments Card */}
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xs space-y-6">
          <div>
            <h2 className="text-lg font-bold text-slate-950 dark:text-white flex items-center gap-2">
              <Building2 className="w-5 h-5 text-teal-600" />
              Practice Locations, Clinics & Reception Desks
            </h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Duty status is verified independently for each affiliated clinic and hospital in Maharashtra.
            </p>
          </div>
              ...
              >
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {aff.facility_name || aff.name || "Healthcare Institution"}
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2.5 shrink-0">
                  ...
                  >
                    ...
                    Call Reception ({aff.reception_phone || "+91 712 2541289"})
                  </a>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xs space-y-4">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            JeevanSetu adheres to strict real-data guarantees. Every record is verified against state medical colleges, IMA registries, and MMC registration ledgers.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            ...
            <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 space-y-1">
              ...
              <p className="text-xs font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                ...
                Masked Clinic / Hospital Reception Routing
              </p>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-normal">
                Personal mobile numbers of medical doctors are never exposed. Calls are routed strictly via verified clinic and hospital reception desks.
              </p>
            </div>
          </div>
        </div>
